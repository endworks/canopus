...
/** Most severe first, which is the order a warning list is read in. */
const SEVERITY = ['Unknown', 'Minor', 'Moderate', 'Severe', 'Extreme'];
  ...
  private alerts(body: WarningsResponse, language: string): WeatherAlert[] {
    ...
    return (
      warnings
        ...
        .sort(
          (a, b) =>
            SEVERITY.indexOf(b.severity) - SEVERITY.indexOf(a.severity) ||
            a.onset - b.onset,
        )
    );
  }
